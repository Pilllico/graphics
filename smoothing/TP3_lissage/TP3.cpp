...
void calc_uniform_mean_curvature(const std::vector<glm::vec3>& vertices,
    ...
    for (unsigned int k = 0; k < one_ring.size(); ++k) {
        glm::vec3 L_U = glm::vec3(0.0f);

        glm::vec3 v = vertices.at(k);
        int n = one_ring.at(k).size();

        for (unsigned int i = 0; i < one_ring.at(k).size(); ++i) {
            glm::vec3 v_i = vertices.at(one_ring.at(k).at(i));
            L_U += v_i;
        }
...
void uniform_smooth(const std::vector<glm::vec3>& vertices,
    std::vector<glm::vec3>& curvature,
    std::vector<glm::vec3>& smoothed_vertices) {

    smoothed_vertices.clear();
    smoothed_vertices.resize(vertices.size());

    for (unsigned int i = 0; i < smoothed_vertices.size(); ++i) {
        smoothed_vertices.at(i) = vertices.at(i) + curvature.at(i) / 2.0f;
    }
}

void calc_triangle_quality(const std::vector<glm::vec3>& vertices,
    ...
    for (unsigned int i = 0; i < tshape.size(); ++i) {
        glm::vec3 v_a = vertices.at(triangles.at(i).at(0));
        glm::vec3 v_b = vertices.at(triangles.at(i).at(1));
        glm::vec3 v_c = vertices.at(triangles.at(i).at(2));

        float e_a = length(v_c - v_b);
        float e_b = length(v_a - v_c);
        float e_c = length(v_b - v_a);

        float area = length(cross(v_c - v_b, v_a - v_c)) / 2;
        float radius = (e_a * e_b * e_c) / (4 * area);
        float min_edge = glm::min(glm::min(e_a, e_b), e_c);

        tshape.at(i) = radius / min_edge; // Note : Min = 1 / V(3)
    }
}

float calc_weights(glm::vec3 v, glm::vec3 v_i, glm::vec3 v_a_i, glm::vec3 v_b_i) {
    float w = 0;

    glm::vec3 a_e_1 = v - v_a_i;
    glm::vec3 a_e_2 = v_i - v_a_i;

    glm::vec3 b_e_1 = v - v_b_i;
    glm::vec3 b_e_2 = v_i - v_b_i;

    float a_i = glm::acos(dot(a_e_1, a_e_2) / (length(a_e_1) * length(a_e_2)));
    float b_i = glm::acos(dot(b_e_1, b_e_2) / (length(b_e_1) * length(b_e_2)));

    return (cos(a_i) / sin(a_i)) + (cos(b_i) / sin(b_i));
}

void calc_mean_curvature(const std::vector<glm::vec3>& vertices,
    std::vector<std::vector<unsigned short> >& one_ring,
    std::vector<std::vector<unsigned short> > triangles,
    std::vector<glm::vec3>& vcurvature) {
    vcurvature.clear();
    vcurvature.resize(vertices.size());

    for (unsigned int k = 0; k < one_ring.size(); ++k) {

        glm::vec3 L_B = glm::vec3(0.0f);
        ...
        for (unsigned int i = 0; i < one_ring.at(k).size(); ++i) {
            ...
            glm::vec3 v_i = vertices.at(one_ring.at(k).at(i));
            glm::vec3 v_a_i = vertices.at(one_ring.at(k).at((i+1)%n));
            glm::vec3 v_b_i = vertices.at(one_ring.at(k).at((i-1+n)%n));

            L_B += (v_i - v);
            w += calc_weights(v, v_i, v_a_i, v_b_i);
        }
...
void weighted_smooth(const std::vector<glm::vec3>& vertices,
    std::vector<glm::vec3>& vcurvature,
    std::vector<glm::vec3>& smoothed_vertices) {
    ...
    for (unsigned int i = 0; i < smoothed_vertices.size(); ++i) {
        smoothed_vertices.at(i) = vertices.at(i) + vcurvature.at(i) / 2.0f;
    }
}

double calc_angle(glm::vec3 v, glm::vec3 v_i, glm::vec3 v_a_i) {

    glm::vec3 a_e_1 = v_i - v;
    glm::vec3 a_e_2 = v_a_i - v;
...
void calc_gauss_curvature(const std::vector<glm::vec3>& vertices,
    std::vector<std::vector<unsigned short> >& one_ring,
    std::vector<std::vector<unsigned short> > triangles,
    std::vector<double>& vgausscurvature) {

    vgausscurvature.clear();
    vgausscurvature.resize(vertices.size());
    ...
    for (unsigned int k = 0; k < one_ring.size(); ++k) {
        ...
        for (unsigned int i = 0; i < one_ring.at(k).size(); ++i) {
            ...
            G -= calc_angle(v, v_i, v_a_i);
        }

        vgausscurvature.at(k) = G;
    }
}
...
int main( void )
{
    ...
    std::vector<glm::vec3> vunicurvature;
    std::vector<glm::vec3> vcurvature;
    std::vector<double> vgausscurvature;
    ...
    for (unsigned int i = 0; i < nb_iterations; ++i) {
        collect_one_ring(indexed_vertices, triangles, one_ring);
        calc_uniform_mean_curvature(indexed_vertices, one_ring, vunicurvature);

        calc_triangle_quality(indexed_vertices, triangles, tshape);
        calc_mean_curvature(indexed_vertices, one_ring, triangles, vcurvature);

        calc_gauss_curvature(indexed_vertices, one_ring, triangles, vgausscurvature);

        if (is_uniform) {
            uniform_smooth(indexed_vertices, vunicurvature, smoothed_vertices);
        }
        else {
            weighted_smooth(indexed_vertices, vcurvature, smoothed_vertices);
        }

        indexed_vertices = smoothed_vertices;
    }
